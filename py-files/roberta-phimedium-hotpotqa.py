...
for i in range(N_rows):
    if rob1[i] > rob2[i]:
        rob_answers.append(first_nli[i])
    else:
        rob_answers.append(second_nli[i])


# SYNTHESIS
pre_answers = []
for i in range(N_rows):
    pre_answers.append(preSynthGeneration(first_queries[i], possibilities[i], answers[i], rob_answers[i], sources[i]))
...
df.to_csv('def/phi-medium/roberta/roberta-naive-2.csv')
